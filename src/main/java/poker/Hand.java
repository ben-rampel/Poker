...
public class Hand implements Comparable<Hand>{
    ...
    private List<Card.Rank> determineCoefficients(){
        ...
        if(rank == handName.fullHouse){
            for(Card.Rank c : cardsAsRanksList) if(Collections.frequency(cardsAsRanksList, c) == 3) {list.add(c); break;}
            for(Card.Rank c : cardsAsRanksList) if(Collections.frequency(cardsAsRanksList, c) == 2) {list.add(c); break;}
            return list;
        }
        if(rank == handName.quads){
            for(Card.Rank c : cardsAsRanksList) if(Collections.frequency(cardsAsRanksList, c) == 4) {list.add(c); break;}
            for(Card.Rank c : cardsAsRanksList) if(Collections.frequency(cardsAsRanksList, c) == 1) {list.add(c); break;}
            return list;
        }
        if(rank == handName.trips){
            for(Card.Rank c : cardsAsRanksList) if(Collections.frequency(cardsAsRanksList, c) == 3) {list.add(c); break;}
            for(Card.Rank c : cardsAsRanksList) if(Collections.frequency(cardsAsRanksList, c) == 1) {list.add(c); }
            return list;
        }
        if(rank == handName.twoPair){
            for(Card.Rank c : cardsAsRanksList) if(Collections.frequency(cardsAsRanksList, c) == 2 && !list.contains(c)) {list.add(c);}
            for(Card.Rank c : cardsAsRanksList) if(Collections.frequency(cardsAsRanksList, c) == 1 && !list.contains(c)) {list.add(c);}
            return list;
        }
